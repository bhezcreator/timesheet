<div class="p-0 space-y-6">
    ...
    @if(!$reports->count())
        ...
        />
    @else
        <x-ui.table :columns="['N°', 'Collaborateur', 'Titre', 'Période', 'Statut', 'Soumis le', 'Heures', 'Pièces jointes', 'Actions']">
            <tbody>
                    ...
                    <tr wire:key="report-{{ $report->id }}" class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 align-top">
                            <x-ui.badge variant="info">
                                @php
                                    $activCount = $report->activities->sum('duration');
                                @endphp
                                {{ floor($activCount) }}h : {{ round(($activCount - floor($activCount)) * 60) }}m
                            </x-ui.badge>
                        </td>
